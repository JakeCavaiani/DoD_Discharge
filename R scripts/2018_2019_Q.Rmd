...
2018-05-30 15:30"),ymd_hm("2018-10-12 10:00"), by = '15 mins'))
...
### export PT data to csv to DoD_Discharge->PT_data->2018
```{r, echo=FALSE, warning=FALSE}
Moose1 <- Moose1.pre.middle.post
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2018-05-29 16:00"),ymd_hm("2018-10-26 15:03"), by = '15 mins'))
...
Moose1$Site <- rep("FRCH", length(Moose1$DateTime))
any(is.na(Moose1))
...
# Export 2018 PT data
```{r, echo=FALSE, warning=FALSE}
moose3comb <- moose3comb[,c(4,6,14)]
names(moose3comb) <- c("DateTime", "Site", "Q")
moose3comb$Site <- rep("MOOS", length(moose3comb$DateTime))
...
any(is.na(Q.daily.2018))

write_csv(Q.daily.2018, here("Predicted_Discharge/2015/Q.daily.2018.csv"))



Q_2018 %>% ggplot() + geom_line(aes(DateTime, Q, color = Site))+ ylab("Predicted Q")
```
...
# Remove jump around July 9th
moos.stream.one.2019 <- moos.stream.one.2019 %>% mutate(across(c(AbsPTDepth), ~ifelse(DateTime > "2019-07-08" & DateTime < "2019-07-10" & AbsPTDepth > 1.05, NA, .)))
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2019-05-31 15:45"),ymd_hm("2019-10-22 09:15"), by = '15 mins'))
...
moos.stream.one.2019 <- na_kalman(moos.stream.one.2019)
...
Moose2comb.2019_final <- na_kalman(Moose2comb.2019_final)
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2019-04-29 14:15"),ymd_hm("2019-10-10 9:15"), by = '15 mins'))
...
poke.final.discharge.2019.final$Site <- rep("POKE", length(poke.final.discharge.2019.final$DateTime))

# Fill missing data
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2019-05-10 14:45"),ymd_hm("2019-10-14 00:00"), by = '15 mins'))
...
Lets remove the high YSI point and make it quadratic.

```{r, echo=FALSE, warning=FALSE}
QSummary.VA.2019.2 <- QSummary.2019 %>% filter(Site =="Vault") %>% filter(Method != "YSI" | Q_Ls < 100) %>% drop_na(Q_Ls)
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2019-06-07 17:45"),ymd_hm("2019-10-11 13:45"), by = '15 mins'))
...
vaul.final.discharge.2019.final <- na_kalman(vaul.final.discharge.2019.final)
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("	
2019-05-21 14:45"),ymd_hm("2019-10-12 23:45"), by = '15 mins'))

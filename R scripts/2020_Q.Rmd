...
strt.stream.2020.url <- ""
strt.stream.2020.url.two <- ""
vaul.stream.2020.url <- ""
...
# Rename columns #
names(moos.stream.one.2020) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WaterPressure")
names(moos.stream.two.2020) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WaterPressure")
...
names(strt.stream.one.2020) <- c("Site", "DateTimeGMT", "Absolute Pressure", "WaterPressure")
names(strt.stream.two.2020) <- c("Site", "DateTimeGMT", "Absolute Pressure", "WaterPressure")

names(vaul.stream.2020) <- c("Site", "DateTimeGMT", "Absolute Pressure", "WaterPressure")

names(poke.stream.one.2020) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WaterPressure")
names(poke.stream.two.2020) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WaterPressure")
...
moos.stream.one.2020$DateTime <- mdy_hms(moos.stream.one.2020$DateTimeGMT, tz = "America/Anchorage")

moos.stream.two.2020$DateTime <- mdy_hms(moos.stream.two.2020$DateTimeGMT, tz = "America/Anchorage")
...
strt.stream.one.2020$DateTime <- mdy_hms(strt.stream.one.2020$DateTimeGMT, tz = "America/Anchorage")

strt.stream.two.2020$DateTime <- mdy_hms(strt.stream.two.2020$DateTimeGMT, tz = "America/Anchorage")


vaul.stream.2020$DateTime <- mdy_hms(vaul.stream.2020$DateTimeGMT, tz = "America/Anchorage")


poke.stream.one.2020$DateTime <- mdy_hms(poke.stream.one.2020$DateTimeGMT, tz = "America/Anchorage")

poke.stream.two.2020$DateTime <- mdy_hms(poke.stream.two.2020$DateTimeGMT, tz = "America/Anchorage")
...
plot(x = moos.stream.one.2020$WaterPressure, y = moos.stream.two.2020$WaterPressure, main = "Moose PT comparison",
     xlab = "Moose1PT", 
     ylab = "Moose2PT")
...
```{r, echo=FALSE, warning=FALSE}
plot(x = strt.stream.one.2020$WaterPressure, y = strt.stream.two.2020$WaterPressure, main = "Stuart PT comparison",
     xlab = "Stuart1 PT", 
     ylab = "Stuart2 PT")
...
```{r, echo=FALSE, warning=FALSE}
plot(x = poke.stream.one.2020$WaterPressure, y = poke.stream.two.2020$WaterPressure, main = "Poker PT comparison",
     xlab = "Poker1 PT", 
     ylab = "Poker2 PT")
...
moos.stream.one.2020 %>% ggplot(aes(DateTime, AbsolutePressure))  + geom_point()
...
plot(moos.stream.one.2020$DateTime, moos.stream.one.2020$WaterPressure, type = 'l')
...
names(moos.stream.one.after) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WLB4", "DateTime", "WaterPressure")
...
#     axes=F, xlab="", ylab="")
#par(new = T)
plot(moos.stream.one.2020.final$DateTime, moos.stream.one.2020.final$WaterPressure, type = 'l')
...
#     axes=F, xlab="", ylab="")
#par(new = T)
plot(moos.stream.two.2020$DateTime, moos.stream.two.2020$WaterPressure, type = 'l')
...
names(moos.stream.two.after) <- c("Site", "DateTimeGMT", "AbsolutePressure", "WLB4", "DateTime", "WaterPressure")
...
moos.stream.two.2020.final %>% filter(DateTime > "2020-08-20" & DateTime < "2020-09-01") %>% ggplot(aes(DateTime, WaterPressure)) + geom_point() + geom_point(data = frch.stream.one.2020, aes(DateTime, WaterPressure - 18))
...
MOOS.2020.dt <- setDT(moos.stream.one.2020.final)

QSummary.MO.2020 <- QSummary.2020 %>% filter(Site =="MOOS")
Moose1comb.2020 <- MOOS.2020.dt[QSummary.MO.2020, on = "DateTime", roll = 'nearest']


moos.formula <- y ~ x
ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Moose1comb.2020) +
...
Moose1comb.2020.2 <- Moose1comb.2020[-c(1,11,13,5,16), ] %>% filter(MeasuredQ_Ls > 500)


moos.formula <- y ~ x
ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Moose1comb.2020.2) +
  geom_point(aes(color = Method), size = 3) +
...
1) Removed the highest YSI

## Predict PT1 Q

```{r, echo=FALSE, warning=FALSE}
MOOS1.lm.2020 <- lm(Moose1comb.2020.2$MeasuredQ_Ls ~ Moose1comb.2020.2$WaterPressure)# + I(Moose1comb.2020.2$WaterPressure^2))

moos.stream.one.2020.final$pred.moos1.Q <- coef(MOOS1.lm.2020)[2] * moos.stream.one.2020.final$WaterPressure+ coef(MOOS1.lm.2020)[1] #+ coef(MOOS1.lm.2020)[3] * I(moos.stream.one.2020.final$WaterPressure^2)

ggplot(aes(x = DateTime, y = pred.moos1.Q), data = moos.stream.one.2020.final) +
  geom_line(color="#A6CEE3", size=1.25) +
  geom_point(data = Moose1comb.2020.2, aes(x = DateTime, y = MeasuredQ_Ls, shape = Method), size=3) +
  theme_classic() +
  ggtitle("Moose1 predicted measured Q") +
  scale_shape_discrete(name = "Method", labels = c("Wading Rod", "Salt Dilution", "")) +
  xlab("Date") +
  ylab("Predicted Discharge") + ylim(-10,50)
...
ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Moose2comb.2020) +
  geom_point(aes(color = Method), size = 3) +
  geom_smooth(method = "lm", se=FALSE) +
  stat_poly_eq(formula = moos.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
Moose2comb.2020.2 <- Moose2comb.2020[-c(13), ]


moos.formula <- y ~ poly(x,2)
ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Moose2comb.2020.2) +
  geom_point(aes(color = Method), size = 3) +
  geom_smooth(method = "lm", se=FALSE) +
  stat_poly_eq(formula = moos.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
```{r, echo=FALSE, warning=FALSE}
MOOS2.lm.2020 <- lm(Moose2comb.2020.2$MeasuredQ_Ls ~ Moose2comb.2020.2$WaterPressure + I(Moose2comb.2020.2$WaterPressure^2))


moos.stream.two.2020.final$pred.moos2.Q <- coef(MOOS2.lm.2020)[2] * moos.stream.two.2020.final$WaterPressure+ coef(MOOS2.lm.2020)[1] + coef(MOOS2.lm.2020)[3] * I(moos.stream.two.2020.final$WaterPressure^2)
...
moos.final.discharge.2020 <- full_join(moos.stream.two.2020.final, moos.stream.one.2020.final, by = c("DateTime"))
moos.final.discharge.2020 <- moos.final.discharge.2020 %>% dplyr::select("Site.x", "DateTime", "pred.moos2.Q", "pred.moos1.Q")
moos.final.discharge.2020$Site.x <- rep("MOOS", length(moos.final.discharge.2020$DateTime))
...
moos.final.discharge.2020$MeanDischarge <- rowMeans(moos.final.discharge.2020[,c(3,4)], na.rm = TRUE)
names(moos.final.discharge.2020) <- c("Site", "DateTime", "pred.moos1.Q", "pred.moos2.Q", "MeanDischarge")
...
ggplot(aes(x = DateTime, y = pred.moos1.Q), data = moos.final.discharge.2020) +
  ...
  geom_point(data = Moose2comb.2020, aes(x = DateTime, y = MeasuredQ_Ls), size=2) +
  theme_classic() +
  #ylim(0, 5000) +
  ggtitle("Moose1(light) & Moose2(dark) predicted all measured Q") +
  ylab("Predicted discharge L/s") +
  xlab("Time") #+ scale_x_datetime(limits = ymd_h(c("2020-06-26 12", "2020-06-27 01", tz = "America/Anchorage")))


```

Something isn't right for PT 2 at the end of June, so lets only use PT1.
```{r, echo=FALSE, warning=FALSE}
moos.final.discharge.2020 <- moos.final.discharge.2020 %>% mutate(across(c(MeanDischarge), ~ifelse(DateTime < "2020-07-01 16:00"  & DateTime > "2020-06-26 00:00", pred.moos1.Q,.)))
### Moose1 (light blue), Moose2 (dark blue), and mean (red) with observed Q.
...
```{r, echo=FALSE, warning=FALSE}
moos.final.discharge.2020_final <- moos.final.discharge.2020[,-c(3:4)]
...
French2.lm.2020.2 <- lm(French2comb.2020.2$MeasuredQ_Ls.x ~ French2comb.2020.2$difference + I(French2comb.2020.2$difference^2))

frch.formula = y~ poly(x,2)
ggplot(aes(x= difference, y = MeasuredQ_Ls.x), data = French2comb.2020.2) +
  geom_point( size = 3) +
...
## Export PT data to csv in DoD_Discharge->Predicted_Discharge->2020
```{r, echo=FALSE, warning=FALSE}
frch.final.discharge.2020 <- French2comb.2020.2[,c(1,3,14)]
...
poke.stream.one.after$WaterPressure <- poke.stream.one.after[, 4] + 0.068
...
#     axes=F, xlab="", ylab="")
#par(new = T)
plot(poke.one.final$DateTime, poke.one.final$WaterPressure, type = 'l')
...
POKE1.lm.2020 <- lm(Poke1comb.2020$MeasuredQ_Ls ~ Poke1comb.2020$WaterPressure)

poke.formula <- y ~ x


ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Poke1comb.2020) +
  geom_point(aes(color = Method), size = 3) +
...
POKE2.lm.2020 <- lm(Poke2comb.2020$MeasuredQ_Ls ~ Poke2comb.2020$WaterPressure)


ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Poke2comb.2020) +
  geom_point(aes(color = Method), size = 3) +
  geom_smooth(method = "lm", se=FALSE) +
  stat_poly_eq(formula = poke.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
POKE2.lm.2020 <- lm(Poke2comb.2020$MeasuredQ_Ls ~ Poke2comb.2020$WaterPressure + I(Poke2comb.2020$WaterPressure^2))
...
```{r, echo=FALSE, warning=FALSE}
Poke1comb.2020$pred.poke1.Q <- coef(POKE1.lm.2020)[2] * Poke1comb.2020$WaterPressure+ coef(POKE1.lm.2020)[1]
...
```{r, echo=FALSE, warning=FALSE}
Poke2comb.2020$pred.poke2.Q <- coef(POKE2.lm.2020)[2] * Poke2comb.2020$WaterPressure+ coef(POKE2.lm.2020)[1] + coef(POKE2.lm.2020)[3] * I(Poke2comb.2020$WaterPressure^2)
...
#     axes=F, xlab="", ylab="")
#par(new = T)
plot(vaul.stream.2020$DateTime, vaul.stream.2020$WaterPressure, type = 'l')
...
#     axes=F, xlab="", ylab="")
#par(new = T)
plot(vaul.stream.2020$DateTime, vaul.stream.2020$WaterPressure)


# Site was visited on 220615: shift up slightly before that so that predicted Q isn't negative
#vaul.stream.2020 <- vaul.stream.2020 %>% mutate(across(WaterPressure, ~ifelse(DateTime < "2020-06-15 16:00", WaterPressure + 0.07, .)))

vaul.stream.2020 %>% filter(DateTime < "2020-06-20") %>% ggplot(aes(DateTime, WaterPressure)) + geom_point()
...
QSummary.VA.2020 <- QSummary.2020 %>% filter(Site =="VAUL") %>% filter(MeasuredQ_Ls < 2000) %>%
  filter(Method != "Wading rod" | MeasuredQ_Ls > 250)

### Rating curve for VAUL PT2 ###
vaul.stream.2020$Site <- "VAUL"

Vaul2comb.2020 <- full_join(vaul.stream.2020, QSummary.VA.2020)
VAUL2.lm.2020 <- lm(Vaul2comb.2020$MeasuredQ_Ls ~ Vaul2comb.2020$WaterPressure)# + I(Vaul2comb.2020$WaterPressure^2))

vaul.formula <- y ~ x

ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Vaul2comb.2020) +
  geom_point(aes(color = Method), size = 3) +
...
```{r, echo=FALSE, warning=FALSE}
Vaul2comb.2020$pred.vaul2.Q <- coef(VAUL2.lm.2020)[2] * Vaul2comb.2020$WaterPressure+ coef(VAUL2.lm.2020)[1] #+ coef(VAUL2.lm.2020)[3] * I(Vaul2comb.2020$WaterPressure^2)
...
ggplot(aes(x = DateTime, y = pred.vaul2.Q), data = Vaul2comb.2020) +
  ...
  xlab("") +
  ylab("Discharge (L/s)") +
  scale_x_datetime(limits = as_datetime(c("2020-05-15", "2020-10-10"))) + ylim (-20,100)
...
STRT1.lm.2020 <- lm(Strt1comb.2020$MeasuredQ_Ls ~ Strt1comb.2020$WaterPressure)

strt.formula <- y ~ x

ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Strt1comb.2020) +
  geom_point(aes(color = Method), size = 3) +
...
STRT2.lm.2020 <- lm(Strt2comb.2020$MeasuredQ_Ls ~ Strt2comb.2020$WaterPressure)

strt.formula <- y ~ x

ggplot(aes(x = WaterPressure, y = MeasuredQ_Ls), data = Strt2comb.2020) +
  geom_point(aes(color = Method), size = 3) +
  geom_smooth(method = "lm", se=FALSE, formula = strt.formula) +
  stat_poly_eq(formula = strt.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
```{r, echo=FALSE, warning=FALSE}
### Filter Stuart ###
Strt1comb.2020$pred.strt1.Q <- coef(STRT1.lm.2020)[2] * Strt1comb.2020$WaterPressure+ coef(STRT1.lm.2020)[1]
...
```{r, echo=FALSE, warning=FALSE}
Strt2comb.2020$pred.strt2.Q <- coef(STRT2.lm.2020)[2] * Strt2comb.2020$WaterPressure+ coef(STRT2.lm.2020)[1]
...
## Export PT data to csv in DoD_Discharge->Predicted_Discharge->2020
```{r, echo=FALSE, warning=FALSE}
strt.final.discharge.2020_final <- Strt1comb.2020 %>% dplyr::select(DateTime, pred.strt1.Q)

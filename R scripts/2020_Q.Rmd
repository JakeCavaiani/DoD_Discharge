...
```{r, echo=FALSE, warning=FALSE}
poke.pt.2020[,c(1,4,5)] %>% pivot_wider(names_from = "Site", values_from = "WaterLevel") %>% ggplot(aes(POKE1, POKE2)) + geom_point()
```
...
```{r, echo=FALSE, warning=FALSE}
moos.final.discharge.2020_final <- moos.final.discharge.2020[,-c(2,4,5)]
names(moos.final.discharge.2020_final) <- c("DateTime", "Site", "Q")
...
```{r, echo=FALSE, warning=FALSE}
# convert to kPa
strt.air.2020 <- strt.air.2020 %>% mutate(across(AirPressure, ~AirPressure*0.1))
...
```{r, echo=FALSE, warning=FALSE}
# Calculate difference
strt.stream.one.2020 <- full_join(strt.stream.one.2020, strt.air.2020, by = DateTime)
strt.stream.one.2020$difference <- strt.stream.one.2020$WaterPressure - strt.stream.one.2020$AirPressure

strt.stream.two.2020 <- full_join(strt.stream.two.2020, strt.air.2020)

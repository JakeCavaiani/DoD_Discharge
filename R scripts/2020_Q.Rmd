...
frch.stream.one.2020$DateTime <- mdy_hms(frch.stream.one.2020$DateTimeGMT, tz = "America/Anchorage")

frch.stream.two.2020$DateTime <- mdy_hms(frch.stream.two.2020$DateTimeGMT, tz = "America/Anchorage")
...
#NEON_air <-loadByProduct(dpID = "DP1.00004.001", site=c("BONA"), startdate="2020-05", enddate="2020-11")
...
#moos.stream.one.2020 %>% ggplot(aes(DateTime, AbsolutePressure))  + geom_point() + geom_point(data = NEON_bp, aes(DateTime, bp_sea + 12)) + ylim(111,116) + scale_x_datetime(limits = ymd_h(c("2020-07-07 00", "2020-07-30 23")))
```
## Raw Moose PT1
```{r, echo=FALSE, warning=FALSE}
...
moos.stream.two.2020.final %>% filter(DateTime > "2020-08-20" & DateTime < "2020-09-01") %>% ggplot(aes(DateTime, WaterLevel)) + geom_point() + geom_point(data = frch.stream.one.2020, aes(DateTime, WaterLevel - 18))
...
moos.formula <- y ~ x
ggplot(aes(x = WaterLevel, y = MeasuredQ_Ls), data = Moose1comb.2020.2) +
  geom_point(aes(color = Method), size = 3) +
...
```{r, echo=FALSE, warning=FALSE}
MOOS1.lm.2020 <- lm(Moose1comb.2020.2$MeasuredQ_Ls ~ Moose1comb.2020.2$WaterLevel)# + I(Moose1comb.2020.2$WaterLevel^2))

moos.stream.one.2020.final$pred.moos1.Q <- coef(MOOS1.lm.2020)[2] * moos.stream.one.2020.final$WaterLevel+ coef(MOOS1.lm.2020)[1] #+ coef(MOOS1.lm.2020)[3] * I(moos.stream.one.2020.final$WaterLevel^2)
...
ggplot(aes(x = WaterLevel, y = MeasuredQ_Ls), data = Moose2comb.2020.2) +
  geom_point(aes(color = Method), size = 3) +
  geom_smooth(method = "lm", se=FALSE) +
  stat_poly_eq(formula = moos.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
```{r, echo=FALSE, warning=FALSE}
MOOS2.lm.2020 <- lm(Moose2comb.2020.2$MeasuredQ_Ls ~ Moose2comb.2020.2$WaterLevel + I(Moose2comb.2020.2$WaterLevel^2))


moos.stream.two.2020.final$pred.moos2.Q <- coef(MOOS2.lm.2020)[2] * moos.stream.two.2020.final$WaterLevel+ coef(MOOS2.lm.2020)[1] + coef(MOOS2.lm.2020)[3] * I(moos.stream.two.2020.final$WaterLevel^2)

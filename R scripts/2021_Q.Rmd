...
moos.stream.one.2021 %>% ggplot(aes(DateTime, AbsolutePressure)) + geom_point(size = 1) + # geom_point(aes(DateTime_alt, AbsolutePressure), color = "red", size = 1)  + 
  geom_point(data = NEON_bp, aes(DateTimeGMT, corPres + 8), color = "dark orange", size = 1)  + 
scale_x_datetime(limits = ymd_hm(c("2021-07-01 00:00", "2021-07-11 23:00"))) +  ylim(107.5,111) + 
  ggtitle("NEON air pressure (orange, value shifted) compared to water pressure \n read in as GMT (black) or AK time (red) for Moose 2021")
```
...
plot(moos.stream.one.2021$DateTime, moos.stream.one.2021$WaterLevel)
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-05-04 09:00"),ymd_hm("2021-09-28 13:00"), by = '15 mins'))
...
```{r, echo=FALSE, warning=FALSE}
# Add any missing time steps
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-06-30 03:00"),ymd_hm("2021-09-26 24:00"), by = '15 mins'))
...
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-05-04 09:00"),ymd_hm("2021-09-26 24:00"), by = '15 mins'))
...
## Export PT data to csv in DoD_Discharge->Predicted_discharge->2021
```{r, echo=FALSE, warning=FALSE}
frch.final.discharge.2021_final <- frch.final.discharge.2021 %>% dplyr::select(Site.y, DateTime, MeanDischarge)
frch.final.discharge.2021_final$Site.y <- "FRCH"
names(frch.final.discharge.2021_final) <- c( "Site", "DateTime", "Q")
...
```{r, echo=FALSE, warning=FALSE}
# Add any missing time steps
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-05-06 09:00"),ymd_hm("2021-09-29 06:30"), by = '15 mins'))
...
## Export PT data to csv in DoD_Discharge->Predicted_discharge->2021
```{r, echo=FALSE, warning=FALSE}
poke.final.discharge.2021_final <- poke.final.discharge.2021.2 %>% dplyr::select(Site.x, DateTime, MeanDischarge)
poke.final.discharge.2021_final$Site.x <- "POKE"
names(poke.final.discharge.2021_final) <- c( "Site", "DateTime", "Q")
...
```{r, echo=FALSE, warning=FALSE}
# Add any missing time steps
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-06-30 05:10"),ymd_hm("2021-09-27 16:10"), by = '15 mins'))
...
strt.stream.one.2021 <- strt.stream.one.2021 %>% mutate(across(c(WaterLevel), ~ ifelse(DateTime > "2021-08-02" & DateTime < "2021-08-04" & WaterLevel > 250.65, NA, .)))
...
strt.stream.one.2021 <- na_kalman(strt.stream.one.2021, maxgap = 10)


strt.stream.one.2021 %>% #filter(DateTime > "2021-08-01" & DateTime < "2021-08-15") %>%
...
strt.stream.two.2021 %>% filter(DateTime > "2021-07-30" & DateTime < "2021-08-15") %>% ggplot(aes(DateTime, WaterLevel)) + geom_point()
```
...
strt.stream.two.2021 <- strt.stream.two.2021 %>% filter(WaterLevel <250.3)

# Remove beaver dam at beginning of August
strt.stream.two.2021 <- strt.stream.two.2021 %>% mutate(across(c(WaterLevel), ~ ifelse(DateTime > "2021-08-02" & DateTime < "2021-08-04" & WaterLevel > 249.68, NA, .)))
strt.stream.two.2021 <- strt.stream.two.2021 %>% mutate(across(c(WaterLevel), ~ ifelse(DateTime > "2021-08-02" & DateTime < "2021-08-10" & WaterLevel < 249.55, NA, .)))

strt.stream.two.2021 <- na_kalman(strt.stream.two.2021, maxgap = 10)


strt.stream.two.2021 %>% ggplot(aes(DateTime, WaterLevel)) + geom_point()
...
```{r, echo=FALSE, warning=FALSE}
# Add any missing time steps
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-05-07 17:00"),ymd_hm("	
2021-09-28 10:30"), by = '15 mins'))
strt.stream.one.2021 <- full_join(strt.stream.one.2021, DateTimeFill)
strt.stream.one.2021 <- na_kalman(strt.stream.one.2021, maxgap = 10)

# Add any missing time steps
DateTimeFill <- data.frame(DateTime = seq(ymd_hm("2021-05-19 17:00"),ymd_hm("2021-09-28 10:30"), by = '15 mins'))
...
I will remove two high and one low YSI points.
### STRT rating curve 2.0 
```{r, echo=FALSE, warning=FALSE}
### Filter STRT ###
QSummary.ST.2021.1 <- QSummary.ST.2021[-c(2,3,22),]
...
#frch.test <- frch.final.discharge.2021 %>% filter(DateTime > "2021-07-30" & DateTime < "2021-08-15")
...
strt.final.discharge.2021_final$Site.y <- "STRT"
...
write_csv(Q_2021, here("Predicted_Discharge/2021/Q_2021.csv"))

Q.daily.2021 = with(Q_2021, tapply(Q, list(day, Site), mean))
Q.daily.2021 = as.data.frame(Q.daily.2021)

write_csv(Q.daily.2021, here("Predicted_Discharge/2021/Q.daily.2021.csv"))
```

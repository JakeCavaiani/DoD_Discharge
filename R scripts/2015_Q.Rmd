...
eielson.atmo.2015$DateTimeAK <- lubridate::round_date(eielson.atmo.2015$DateTimeAK, "5 minutes")
...
moos.stream %>% filter(DateTimeAK < "2015-06-15") %>% ggplot(aes(DateTimeAK, AbsolutePressure)) + geom_point() + geom_point(data = eielson.atmo.2015, aes(DateTimeAK, AirPressureCorrectedMOOS))
...
moos.stream <- moos.stream %>% filter(DateTimeAK <= "2015-09-21 15:50:00") %>%
  mutate(across(c(AbsolutePressure), 
...
moos.stream$site <- "MOOS"

MOOS.2015.one <- left_join(moos.stream, eielson.atmo.2015, by = "DateTimeAK")
...
MOOS.2015.one$AirPressureCorrectedMOOS <- na_kalman(MOOS.2015.one$AirPressureCorrectedMOOS)
...
```{r, echo=FALSE, warning=FALSE}
#join the atmospheric and water pressure together 
frch.stream$site <- "FRCH"


FRCH.2015.one <- left_join(frch.stream, eielson.atmo.2015, by = "DateTimeAK")
FRCH.2015.one$AirPressureCorrectedFRCH <- na_kalman(FRCH.2015.one$AirPressureCorrectedFRCH)

FRCH.2015.one %>% ggplot(aes(DateTimeAK, AbsolutePressure)) + geom_point() + geom_point(aes(DateTimeAK, AirPressureCorrectedFRCH))

# Water pressure - atmospheric pressure
FRCH.2015.one$difference <- FRCH.2015.one$AbsolutePressure - FRCH.2015.one$AirPressureCorrectedFRCH

FRCH.2015.one %>% ggplot(aes(DateTimeAK, difference)) + geom_point()
...
FRCH.2015.one.dt <- subset(FRCH.2015.one.dt)
...
MOOS.2015.one.dt <- full_join(MOOS.2015.one.dt, DateTimeFill)
MOOS.2015.one.dt <- MOOS.2015.one.dt[order(MOOS.2015.one.dt$DateTimeAK),]
MOOS.2015.one.dt <- na_kalman(MOOS.2015.one.dt)

MOOS.2015.one.dt <- MOOS.2015.one.dt %>% group_by(DateTimeAK) %>% summarise( pred.moos.Q = mean( pred.moos.Q))
any(duplicated(MOOS.2015.one.dt$DateTimeAK))

MOOS.2015.one.dt$diff <- MOOS.2015.one.dt$DateTimeAK - lag(MOOS.2015.one.dt$DateTimeAK)
unique(MOOS.2015.one.dt$diff)
MOOS.2015.one.dt %>% ggplot(aes(DateTimeAK, diff)) + geom_point()

# French
DateTimeFill <- data.frame(DateTimeAK = seq(ymd_hm("2015-05-18 15:15", tz = "America/Anchorage"),ymd_hm("2015-09-21 12:45", tz = "America/Anchorage"), by = '15 mins'))
FRCH.2015.one.dt <- full_join(FRCH.2015.one.dt, DateTimeFill)
FRCH.2015.one.dt <- FRCH.2015.one.dt[order(FRCH.2015.one.dt$DateTimeAK),]
FRCH.2015.one.dt <- na_kalman(FRCH.2015.one.dt)

FRCH.2015.one.dt <- FRCH.2015.one.dt %>% group_by(DateTimeAK) %>% summarise( pred.frch.Q = mean( pred.frch.Q))
any(duplicated(FRCH.2015.one.dt$DateTimeAK))

FRCH.2015.one.dt$diff <- FRCH.2015.one.dt$DateTimeAK - lag(FRCH.2015.one.dt$DateTimeAK)
unique(FRCH.2015.one.dt$diff)
FRCH.2015.one.dt %>% ggplot(aes(DateTimeAK, diff)) + geom_point()
...
```{r}
FRCH.2015.one.dt <- FRCH.2015.one.dt[,-3]
FRCH.2015.one.dt$Site <- "FRCH"
names(FRCH.2015.one.dt) <- c("DateTimeAK", "Q", "Site")
MOOS.2015.one.dt <-  MOOS.2015.one.dt[,-3]
MOOS.2015.one.dt$Site <- "MOOS"
...
Q_2015$DateTimeAK <- lubridate::round_date(Q_2015$DateTimeAK, "15 minutes")
Q_2015 <- Q_2015 %>% group_by(Site, DateTimeAK) %>% summarise(Q = mean(Q))

# Fill gaps in data
DateTimeFill <- data.frame(DateTimeAK = seq(ymd_hm("	
2015-05-14 00:00", tz = "America/Anchorage"),ymd_hm("2015-09-22 00:00", tz = "America/Anchorage"), by = '15 mins'))
DateTimeFill_all <- data.frame(Site = c(rep("FRCH", length(DateTimeFill$DateTimeAK)),rep("MOOS", length(DateTimeFill$DateTimeAK))), DateTimeAK = rep(DateTimeFill$DateTimeAK, 2))
Q_2015 <- full_join(Q_2015, DateTimeFill_all)
Q_2015 <- Q_2015[order(Q_2015$DateTimeAK),]

# Fill in gaps up to 15 x 10 minutes
Q_2015 <- Q_2015 %>% group_by(Site) %>% summarise(Q = na_kalman(Q, maxgap = 50),
                                                  DateTimeAK = DateTimeAK) %>% filter(DateTimeAK < "2022-09-22")
...
write.csv(Q.daily.2015, here("Predicted_Discharge/2015/Q.daily.2015.csv"))

# Plot
Q_2015 %>% ggplot() + geom_line(aes(DateTimeAK, Q, color = Site))+ ylab("Predicted Q")

# Check time step intervals
Q_2015_check <- Q_2015 %>% group_by(Site) %>% summarise(diff = DateTimeAK - lag(DateTimeAK),
                                                        DateTimeAK = DateTimeAK)
unique(Q_2015_check$diff)

Q_2015_check %>% ggplot(aes(DateTimeAK, diff, color = Site)) + geom_point()# + ylim(0,60)

# Check length of consecutive NA's
Q_2015_check_NA <- Q_2015 %>% mutate(Yes_NA = ifelse(Q %in% NA, "Y", "N"))

Q_2015_check_NA %>% #filter(Site == "VAUL") %>% 

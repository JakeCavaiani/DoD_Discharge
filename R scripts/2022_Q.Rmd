...
NEON_bp$DateTimeAK <- NEON_bp$startDateTime
...
NEON_bp$bp_sea <- na.approx(NEON_bp$bp_sea, rule = 2)
NEON_bp %>% filter(DateTimeAK > "2022-07-25", DateTimeAK < "2022-07-30") %>% ggplot(aes(DateTimeAK, bp_sea)) + geom_point()
...
length(is.na(NEON_bp$bp_sea))
any(duplicated(NEON_bp$DateTimeAK))

NEON_bp$bp_sea <- na.approx(NEON_bp$bp_sea, rule = 2)

NEON_bp %>% ggplot(aes(DateTimeAK, bp_sea)) + geom_point()
```


```{r}
# Compare NEON and Eielson sea level pressure
eielson.atmo.2022.compare$sea_level_pressure_kpa <- eielson.atmo.2022.compare$sea_level_pressure*3.38639
NEON_bp %>% ggplot(aes(DateTimeAK, bp_sea)) + geom_point() + geom_point(data = eielson.atmo.2022.compare, aes(DateTimeAK, sea_level_pressure_kpa), color = "red") 
...
2022-05-01 00:00", tz = "America/Anchorage"),ymd_hm("2022-10-15 00:00", tz = "America/Anchorage"), by = '15 mins'))
NEON_bp <- full_join(NEON_bp, DateTimeFill)
Air_combo <- left_join(NEON_bp, eielson.atmo.2022.compare[,c(1,15)])
Air_combo$bp_combo <- rowMeans(Air_combo[,c(2,3)], na.rm = TRUE)
Air_combo$bp_combo <- na_kalman(Air_combo$bp_combo, type = "level")


Air_combo %>% filter(DateTimeAK > "2022-07-01" & DateTimeAK < "2022-08-01") %>% 
  ggplot(aes(DateTimeAK, bp_combo)) + geom_point(size = 2) #+ geom_point(aes(DateTimeAK, bp_sea), color = 2, size = 0.2) + geom_point(aes(DateTimeAK, sea_level_pressure_kpa), color = 3, size = 0.2)# units are kilopascals, corrected to sea level



# Convert to elevation using formula from "Barometric Pressure conversion for DO calibration.xls"
# MOOS 
NEON_bp$AirPressureCorrectedMOOS <- (NEON_bp$bp_sea*7.50062 - (2.5*574/100)) / 7.50062

# POKE 
NEON_bp$AirPressureCorrectedPOKE <- (NEON_bp$bp_sea*7.50062 - (2.5*760.96/100)) / 7.50062

# STRT
NEON_bp$AirPressureCorrectedSTRT <- (NEON_bp$bp_sea*7.50062 - (2.5*820/100)) / 7.50062

# FRCH
NEON_bp$AirPressureCorrectedFRCH <- (NEON_bp$bp_sea*7.50062 - (2.5*601/100)) / 7.50062

# VAUL
NEON_bp$AirPressureCorrectedVAUL <- (NEON_bp$bp_sea*7.50062 - (2.5*688.8/100)) / 7.50062

any(is.na(NEON_bp))
```
Trim merged air pressure data
```{r}
Air_combo <- Air_combo %>% mutate(across(c(bp_combo), ~ifelse(DateTimeAK > "2022-05-15 15:00" & DateTimeAK < "2022-05-17" & bp_combo > 101.1, NA, .)))

Air_combo %>% filter(DateTimeAK > "2022-05-20" & DateTimeAK < "2022-06-01") %>% 
  ggplot(aes(DateTimeAK, bp_combo)) + geom_point(size = 2)
```
...
moos.atmo.2022 %>% filter(AirPressure > 96.8) %>% ggplot(aes(x = DateTimeAK, y = AirPressure)) +
  geom_point() + geom_point(data = NEON_bp, aes(DateTimeAK, AirPressureCorrectedMOOS), color = "red")
...
moos.stream.two.2022 <- moos.stream.two.2022 %>%
  mutate(across(c(WaterPressure), 
                ~ifelse(DateTimeAK >= "2022-09-28" & DateTimeAK <= "2022-09-28 17:00" & WaterPressure < 100.2, NA, .)))
...
MOOS.2022 <- full_join(moos.final.pressure.2022, NEON_bp, by = "DateTimeAK") %>% select("DateTimeAK", "AirPressureCorrectedMOOS", "WaterPressure")

# Fill gaps in data


DateTimeFill <- data.frame(DateTimeAK = seq(ymd_hm("2022-05-09 10:00", tz = "America/Anchorage"),ymd_hm("2022-10-10 16:00", tz = "America/Anchorage"), by = '15 mins'))
MOOS.2022 <- full_join(MOOS.2022, DateTimeFill, by = "DateTimeAK")
MOOS.2022$WaterPressure <- na_kalman(MOOS.2022$WaterPressure, maxgap = 9)
MOOS.2022$AirPressureCorrectedMOOS <- na_kalman(MOOS.2022$AirPressureCorrectedMOOS)
...
MOOS1.lm.2022 <- lm(Moose1comb.2022.2$MeasuredQ_Ls ~ Moose1comb.2022.2$difference + I(Moose1comb.2022.2$difference^2))

MOOS.2022.dt$pred.moos1.Q <- coef(MOOS1.lm.2022)[3] * I(MOOS.2022.dt$difference^2) + coef(MOOS1.lm.2022)[2] * MOOS.2022.dt$difference+ coef(MOOS1.lm.2022)[1]
...
MOOS.2022.dt <- na_kalman(MOOS.2022.dt, maxgap = 9)
...
poke.final.pressure.2022 <- poke.final.pressure.2022 %>%
  ...
  select("DateTimeAK", "WaterPressure.merged")
...
poke.final.pressure.2022 %>% #filter(DateTimeAK > "2022-09-26 14:40" & DateTimeAK < "2022-09-27") %>% 
...
POKE.2022 <- full_join(NEON_bp, poke.final.pressure.2022, by = "DateTimeAK") %>% 
  select(DateTimeAK, AirPressureCorrectedPOKE, WaterPressure.merged)
...
# Water pressure - atmospheric pressure 
DateTimeFill <- data.frame(DateTimeAK = seq(ymd_hm("	
2022-05-18 00:45", tz = "America/Anchorage"),ymd_hm("2022-10-04 06:00", tz = "America/Anchorage"), by = '15 mins'))
POKE.2022 <- full_join(POKE.2022, DateTimeFill, by = "DateTimeAK")

POKE.2022$WaterPressure.merged <- na_kalman(POKE.2022$WaterPressure.merged, maxgap = 9)
any(is.na(POKE.2022$AirPressureCorrectedPOKE))
POKE.2022$AirPressureCorrectedPOKE <- na_kalman(POKE.2022$AirPressureCorrectedPOKE)
any(is.na(POKE.2022$AirPressureCorrectedPOKE))
...
```{r, warning = FALSE, message = FALSE}
# join the two atmospheric and water pressure together
STRT.2022 <- full_join(eielson.atmo.2022.compare, strt.final.pressure.2022, by = "DateTimeAK") %>% select("DateTimeAK", "AirPressureCorrectedSTRT", "WaterPressure")
STRT.2022$AirPressureCorrectedSTRT <-  na.approx(STRT.2022$AirPressureCorrectedSTRT, rule = 2) 
...
STRT.2022 %>% #filter(DateTimeAK >"2022-09-26 10:00:00" & DateTimeAK < "2022-09-26 20:00:00") %>%

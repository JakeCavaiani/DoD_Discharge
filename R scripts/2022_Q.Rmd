...
2022-05-01 00:00"),ymd_hm("2022-10-15 00:00"), by = '5 mins'))
NEON_bp <- full_join(NEON_bp, DateTimeFill)
Air_combo <- left_join(NEON_bp, eielson.atmo.2022.compare)
Air_combo <- Air_combo %>% mutate(bp_combo = ifelse(bp_sea %in% NA, sea_level_pressure_kpa,. ))

NEON_bp <- na_kalman(NEON_bp, type = "level")
any(is.na(NEON_bp))

NEON_bp %>% ggplot(aes(DateTimeAK, bp_sea)) + geom_point() # units are kilopascals, corrected to sea level

...
MOOS.2022 <- full_join(moos.final.pressure.2022, atmo.pt.2022.1, by = "DateTimeAK") %>% select("DateTimeAK", "AirPressureCorrectedMOOS", "WaterPressure")

# Fill gaps in data
MOOS.2022$AirPressureCorrectedMOOS <- na.approx(MOOS.2022$AirPressureCorrectedMOOS, rule = 2)
...
I will remove three YSI points.

```{r, warning = FALSE, message = FALSE}
# Remove outlier Q points
Moose1comb.2022.2 <- Moose1comb.2022 %>% filter(difference > 4 | MeasuredQ_Ls < 1000) %>% filter(MeasuredQ_Ls > 300)
...
MOOS1.lm.2022 <- lm(Moose1comb.2022.3$MeasuredQ_Ls ~ Moose1comb.2022.3$difference + I(Moose1comb.2022.3$difference^2))
...
eielson.atmo.2022.compare$AirPressureCorrectedPOKE <- na_kalman(eielson.atmo.2022.compare$AirPressureCorrectedPOKE)
POKE.2022 <- full_join(eielson.atmo.2022.compare, poke.final.pressure.2022, by = "DateTimeAK") %>% 
...
# Water pressure - atmospheric pressure 
POKE.2022$AirPressureCorrectedPOKE <- na.approx(POKE.2022$AirPressureCorrectedPOKE, rule = 2)
...
POKE.2022$AirPressureCorrectedPOKE <- na_kalman(POKE.2022$AirPressureCorrectedPOKE, maxgap = 9)

...
ggplot(aes(x = difference, y = MeasuredQ_Ls), data = Stuart1comb.2022) +
  ...
  ggtitle("Stuart all measured Q")
```
Lefts try removing that low difference, high measured Q YSI point:
```{r}
...
atmo.pt.frch <- full_join(frch.stream.one.2022, frch.stream.two.2022, by = "DateTimeAK")
...
# They look the same and PT1 is more complete, so we will use that one.
```{r}
...
```{r}
vaul.final.pressure.2022.2 <- mutate(data = vaul.final.pressure.2022, DateTimeAK = DateTimeAK + 28800) #Shift time by 8 hours - the difference between AK time and GMT
...
VAUL.2022.2 <-  VAUL.2022.2[ , -which(names(VAUL.2022) %in% c("TempC"))] # removing columns I do not need

VAUL.2022.2$Site.x <- "VAUL"

...
moos.stream.one.2022 <- moos.stream.one.2022[, -c(5:9)] # removing columns that aren't date/abs pressure and temp
...
moos.1.after <- moos.stream.one.2022[c(8405:40000), ]
...
moos.1.before2 <- moos.stream.one.2022[c(1:21686), ]
moos.1.after2 <- moos.stream.one.2022[c(21695:40000), ]
moos.1.after2$WaterPressure <- moos.1.after2[, 2] - 6.1
moos.stream.one.2022 <- full_join(moos.1.before2, moos.1.after2)
...
moos.2.after <- moos.stream.two.2022[c(8403:40000), ]
...
moos.2.before2 <- moos.stream.two.2022[c(1:30020), ] 
moos.2.after2 <- moos.stream.two.2022[c(30032:40000), ]
moos.stream.two.2022[30032, 2] - moos.stream.two.2022[30020, 2]  # 11.506
moos.2.after2$WaterPressure <- moos.2.after2[, 2] - 11.506
moos.stream.two.2022 <- full_join(moos.2.before2, moos.2.after2)


#moos.2.before3 <- moos.stream.two.2022[c(1:21686), ] 
#moos.2.after3 <- moos.stream.two.2022[c(21687:40000), ]
#moos.stream.two.2022[21687, 2] - moos.stream.two.2022[21686, 2]  # 5.389. Raise by 0.5 because of gap?
...
ggplot(data = moos.stream.two.2022, aes(x = DateTimeAK, y = WaterPressure)) +
  geom_point() +
  theme_classic() + geom_point(data = moos.stream.one.2022, aes(x = DateTimeAK, y = WaterPressure), col = 2)
...
# KLJ
AA <- MOOS.2022 %>% filter(DateTimeAK < "2022-07-29 12:00:00") %>% summarise(meandiff = mean(WaterPressure.x - WaterPressure.y, na.rm = TRUE))

BB <- MOOS.2022 %>% filter(DateTimeAK > "2022-07-29 12:00:00") %>% summarise(meandiff = mean(WaterPressure.x - WaterPressure.y, na.rm = TRUE))
BB-AA
...
ggplot(moos.final.pressure.2022, aes(WaterPressure.x, WaterPressure.y)) + geom_point() + ggtitle("Moose PT comparison") + xlab("Moose1PT") +  ylab ("Moose2PT")
...
# Find the closest pressure time to the Q measurement
for(i in 1:length(QSummary.MO.2022)){
  QSummary.MO.2022$Date_closest[i] = as.character(MOOS.2022$DateTimeAK[which.min(QSummary.MO.2022$DateTimeAK[i]-MOOS.2022$DateTimeAK)])
}
QSummary.MO.2022$Date_closest <- strptime(QSummary.MO.2022$Date_closest, "%Y-%m-%d %H:%M:%S")

# Remove bad YSI points
#QSummary.MO.2022 <- QSummary.MO.2022[-c(5),]

# Add the pressure data
Moose1comb.2022 <- left_join(QSummary.MO.2022, MOOS.2022)
...
```
This is the rating curve. We are missing 5/9 and 5/17 YSI because we didn't have our Pressure Transducers in yet. We are missing 8/29 wading rod due to downloading data off before taking a Q measurement so the times are off by a couple hours. 
...
```
This is the rating curve with just PT2 but we are missing 7/26 discharge measurements because there is a gap in the water level data from 7/23-8/10 and then we are missing the 9/2 discharge measurements due to downloading off the data before a discharge measurement was taken 
...
```
something going on in the middle here 
